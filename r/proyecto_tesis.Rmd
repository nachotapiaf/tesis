...
*	Objetivo general: Conocer la relación entre los indicadores de cultura política y el apoyo a la democracia en la población chilena entre 18-40 años durante el año 2021.
...
Por otra parte, el ciudadano que es identificado como ‘asertivo’ es uno, digamos, opuesto a este ciudadano ‘leal’. Sus valores y prioridades enfatizan su voz y participación en vez de prioridades asociadas a la seguridad o el orden. Responden a valores postmaterialistas según lo entiende Inglehart (1997). Su orientación hacia la autoridad se basa en distanciarse de esta en su trabajo o en su familia así como de la política; tiene una baja confianza en las instituciones. Sin embargo, apoya fuertemente los principios de la democracia pero tiene un débil apoyo por su práctica; son demócratas insatisfechos. De esta forma identifican la democracia con conceptos como la participación y expresión como componentes dominantes. Por último, tienen una fuerte afinidad con la acción no violenta que desafía a las elites. (Dalton y Welzel, 2014)
...
## 7.2 carga de datos y visualización 
```{r include=FALSE}
datos_proc <- readRDS("../output/datos/datos_proc.rds")
```

# 8. Presentación de variables 
```{r echo=FALSE}
view_df(datos_proc)
```

# 9. Creación de objeto encuesta
```{r echo=FALSE}
...
# 10. Gráficos bivariados: apoyo a la democracia según sexo
```{r echo=FALSE}
ggplot(datos_proc) + # apoyo a la democracia según sexo
  geom_mosaic(aes(x=product(apoyo_dem_bin, sexo), fill=sexo)) +
...
# 10.1 Gráficos bivariados: apoyo a la democracia según nivel educacional
```{r echo=FALSE}
ggplot(datos_proc) + # apoyo a la democracia según nivel educacional
  geom_mosaic(aes(x=product(apoyo_dem_bin, nivel_educ), fill=nivel_educ)) +
  geom_label(data = layer_data(last_plot(), 1),
             aes(x = (xmin + xmax)/ 2,
                 y = (ymin + ymax)/ 2,
                 label = paste0(round((.wt/sum(.wt))*100,1),"%"))) +
...
# 10.3 Gráficos bivariados: apoyo a la democracia según edad
```{r echo=FALSE}
ggplot(datos_proc) + # apoyo a la democracia según edad_tramo
  geom_mosaic(aes(x=product(apoyo_dem_bin, edad_tramo), fill=edad_tramo)) +
  geom_label(data = layer_data(last_plot(), 1),
             aes(x = (xmin + xmax)/ 2,
                 y = (ymin + ymax)/ 2,
                 label = paste0(round((.wt/sum(.wt))*100,1),"%"))) +
  labs(y = "Apoya la democracia",
       x = "Edad por tramo") +
  scale_fill_discrete(name = "",
                      labels = c("Joven","Adulto joven","Adulto"))+
  scale_y_continuous(breaks = c(0,1),
                     labels = c("Apoya la democracia","Apoya el autoritarismo")) +
  theme(legend.position="bottom")
frq(datos_proc$apoyo_dem)
...
# 10.2 Test de anova para variables categóricas y continuas
```{r echo=FALSE}
sjp.aov1(datos_proc$edad, datos_proc$apoyo_dem, 
         title = "Test ANOVA para diferencias en apoyo a la democracia según edad",
         axis.title = "Apoyo a la democracia")
  theme_sjplot2()
```

# 10.3 Gráfico de correlaciones 
```{r echo=FALSE}
data.frame(as_factor(sample(datos_proc$confianza_pdte, replace = TRUE)), #Transformamos variables
           as_factor(sample(datos_proc$confianza_gob, replace = TRUE)), #para hacerlas compatibles
           as_factor(sample(datos_proc$confianza_pp, replace = TRUE)), #con la función sjp.chi2
           as_factor(sample(datos_proc$confianza_congreso, replace = TRUE)),
           as_factor(sample(datos_proc$confianza_social, replace = TRUE)),
           as_factor(sample(datos_proc$apoyo_dem, replace = TRUE)),
           as_factor(sample(datos_proc$opina_rrss, replace = TRUE)),
           as_factor(sample(datos_proc$interes_politica, replace = TRUE)),
           as_factor(sample(datos_proc$informa_politica, replace = TRUE)),
           as_factor(sample(datos_proc$habla_politica, replace = TRUE)),
           as_factor(sample(datos_proc$nivel_educ, replace = TRUE)),
           as_factor(sample(datos_proc$satis_dem, replace = TRUE))) %>% 
  sjp.chi2(., 
           title = "Test de Chi2 (p-valores)",
           axis.labels = c("Apoyo a la democracia", #Indicamos etiquetas eje Y
                           "Confianza en el Presidente",
                           "Confianza en el Congreso",
                           "Confianza en el Gobierno",
                           "Confianza en los Partidos Políticos",
                           "Confianza social",
                           "Opina por RR.SS sobre temas públicos ",
                           "Interés en política",
...
# 10. Modelo de regresión logística binaria

## 10.1. Construcción múltiple modelos logísticos

En este apartado se crean los modelos: 1, 2, 3, 4, 5,6 y 7 junto con el modelo 7 con el objeto encuesta.

```{r modelos_logisticos, echo=FALSE, message=FALSE, warning=FALSE}

modelo0 <- glm(apoyo_dem_bin ~ 1, family = binomial(link = "logit"), data = datos_proc)
modelo1 <- glm(apoyo_dem_bin ~ sexo, family = binomial(link = "logit"), data = datos_proc) # modelo binario
modelo2 <- glm(apoyo_dem_bin ~ edad, family = binomial(link = "logit"), data = datos_proc) # modelo continuo
modelo3 <- glm(apoyo_dem_bin ~ edad + sexo + nivel_educ, family = binomial(link = "logit"), data = datos_proc) # modelo multivariado
modelo4 <- glm(apoyo_dem_bin ~ edad + sexo + nivel_educ + interes_politica, family = binomial(link = "logit"), data = datos_proc) # modelo multivar
modelo5 <- glm(apoyo_dem_bin ~ edad + sexo + nivel_educ + interes_politica + opina_rrss, family = binomial(link = "logit"), data = datos_proc)
modelo6 <- glm(apoyo_dem_bin ~ edad + sexo + nivel_educ + interes_politica + opina_rrss + informa_politica, family = binomial(link = "logit"), data = datos_proc)
modelo7 <- glm(apoyo_dem_bin ~ edad + sexo + nivel_educ + interes_politica + opina_rrss + informa_politica + habla_politica, family = binomial(link = "logit"), data = datos_proc)
modelo7survey <- svyglm(apoyo_dem_bin ~ edad + sexo + nivel_educ + interes_politica + opina_rrss + informa_politica + habla_politica, family = binomial(link = "logit"), design = elsoc_objeto, data = datos_proc)
```
...
***Gráfico1 modelo 5 de valores predichos apoyo a la democracia según sexo***
...
***Gráfico2 modelo 5 de valores predichos apoyo a la democracia según nivel educacional***
...
### e. Comparación y reporte estimación modelo 7 con R base y modelo 7 con diseño complejo de encuestas.

***Reporte con summary de modelo 7 con R base y diseño complejo***

Primero se reporta el modelo 7 sin pesos muestrales:
...
***Reporte con tab_model de modelo 7 con R base y diseño complejo***

Asimismo, se reporta la tabla del modelo 7 con R base:
...
Para luego compararla con la tabla del modelo 7 con diseño complejo:
