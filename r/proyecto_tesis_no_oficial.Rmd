---
title: "Cultura política y apoyo a la democracia en Chile durante 2021"
subtitle: 'Un análisis desde las juventudes'
...
view_df(datos_proc)
```
# 9. Creación de objeto encuesta para trabajar con diseño muestral complejo de la ELSOC
...
## 10.1 Descriptivos para "Confianza institucional"
```{r, confianza institucional}
plot_stackfrq(datos_proc[,c("confianza_gob","confianza_pp","confianza_congreso", "confianza_pdte")]) + theme(legend.position="bottom")
```

## 10.2 Descriptivos para "Interés y conocimiento político"
```{r, conocimiento}
plot_stackfrq(datos_proc[c("opina_rrss","informa_politica", "habla_politica")]) + theme(legend.position="bottom")
```

## 10.3 Descriptivos para dimensión "Percepciones hacia la democracia": Satisfacción con la democracia
```{r echo=FALSE}
frq(datos_proc$satis_dem,
  ...
  kable(caption = "Distribución de satisfacción con la democracia", #Incorporamos título
        format = "html", #Especificamos output en HTML
        col.names = c("satis_dem", # Definimos el nombre de las columnas
                      "F. absoluta",
                      "F. relativa",
                      "F. rel. acumulada"),
...
## 10.4 Descriptivos para dimensión "Percepciones hacia la democracia": Apoyo a la democracia
```{r}
frq(datos_proc$apoyo_dem,
  ...
  mutate_at(vars(ends_with("prc")), # A las variables que terminen con "prc"
                 ~(paste(., "%"))) %>% #Incorporamos porcentaje con paste()
  kable(caption = "Distribución de apoyoa a la democracia", #Incorporamos título
        format = "html", #Especificamos output en HTML
        col.names = c("apoyo_dem", # Definimos el nombre de las columnas
                      "F. absoluta",
                      "F. relativa",
                      "F. rel. acumulada"),
        position = "center") %>% # Especificamos que la tabla se muestre al centro
...
### 10.5 Tablas de continengencia de apoyo a la democracia según tramo edad
```{r}
